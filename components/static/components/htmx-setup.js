 ...
 * обычными переходами.
 *
 * 1. Скрипты страницы — вкладки, выпадающие списки, даты, поля файлов,
 *    высота списка в 25 строк (rows.js), подсветка строки с краткой
 *    карточкой (preview.js) — один раз при загрузке находят свои
 *    элементы и оформляют их. Кусок, который
...
(function () {
  ...
  htmx.onLoad(function (root) {
    var oy = window.OY || {};
    ["tabs", "selects", "dates", "files", "rows", "previews"].forEach(function (name) {
